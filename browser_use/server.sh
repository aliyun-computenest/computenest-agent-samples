...
stop() {
    ...
    local serve_pid_file="$FRONTEND_DIR/.serve.pid"
    local dev_pid_file="$FRONTEND_DIR/.frontend.pid"

    if [ -f "$serve_pid_file" ]; then
        stop_process "前端静态服务器" "$serve_pid_file"
    elif [ -f "$dev_pid_file" ]; then
        stop_process "前端开发服务器" "$dev_pid_file"
    fi

    echo "browser_use 已停止"
}
...
start() {
    ...
    if [ -n "$public_ip" ]; then
        export BASE_URL="http://${public_ip}:8090/process"
        echo "前端 BASE_URL：$BASE_URL"
    fi
    ...
    if [ -f "$serve_pid_file" ]; then
        local old_serve_pid
        old_serve_pid=$(cat "$serve_pid_file")
        kill "$old_serve_pid" 2>/dev/null || true
        rm -f "$serve_pid_file"
        sleep 1
    fi

    nohup npx serve dist -l 5173 > /dev/null 2>&1 &
    echo $! > "$serve_pid_file"
