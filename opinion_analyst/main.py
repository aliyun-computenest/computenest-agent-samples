# -*- coding: utf-8 -*-
"""Opinion Analyst Agent Service 入口。

启动时初始化 Credential 与 Agent「Opinion Analyst」；Workspace 预置百炼 WebSearch MCP。
"""
...
from tools.prompts import OPINION_DEEP_RESEARCH_PROMPT

logging.basicConfig(
    level=os.getenv("LOG_LEVEL", "INFO").upper(),
    format="%(asctime)s %(levelname)s [%(name)s] %(message)s",
)
logger = logging.getLogger("opinion_analyst")
...
AGENT_ID = "opinion_analyst"
AGENT_NAME = "Opinion Analyst"
DEFAULT_SESSION_NAME = "Default"
SYSTEM_PROMPT = OPINION_DEEP_RESEARCH_PROMPT
...
def build_app():
    """构建 Opinion Analyst Agent Service。"""
