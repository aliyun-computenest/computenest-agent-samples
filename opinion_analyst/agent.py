# -*- coding: utf-8 -*-
"""舆情 Deep Research：子任务分解 → 多轮 MCP 联网搜索 → 续搜判断 → 中间摘要 → 终稿流式输出。"""
...
# 非深度研究回合：直接对话（与 main 中长报告 sys_prompt 分离）
_CONVERSATIONAL_REPLY_SYS = """你是舆情深度研究助手（本服务示例）。
用户**当前这一轮**不需要「多轮联网检索 + 结构化舆情长报告」。请**直接、简洁、友好**地回答（几句到一小段即可）。
若用户其实是在做舆情/品牌/事件分析，可礼貌提示：请说明要关注的**主体（品牌/人物/事件等）**、**时间范围**或关切点。"""


class OpinionRouteDecision(BaseModel):
    """由轻量路由模型判断：是否走舆情深度研究编排。"""

    need_deep_research: bool = Field(
        description="为 true 时执行多轮联网检索与舆情长报告；为 false 时仅简短对话回复",
    )
    reason: str = Field(default="", description="判断理由，一两句中文即可")


class OpinionDecompose(BaseModel):
    knowledge_gaps: str = Field(description="当前对议题的不确定点与待核实点")
    working_plan: str = Field(description="3～6 步可执行的研究计划（中文）")
    initial_queries: list[str] = Field(
        description="3～8 条适合中文互联网的搜索查询词",
        min_length=1,
        max_length=10,
    )
...
class OpinionDeepResearchAgent(ReActAgent):
    def __init__(
        self,
        name: str,
        sys_prompt: str,
    ...
    async def _phase_route(self) -> OpinionRouteDecision:
        """一次轻量 JSON 调用，区分闲聊与「需要舆情深度研究」。"""
        sys_txt = (
            "你是**路由判断器**，只输出**一个 JSON 对象**，不要 Markdown 围栏或其它文字；"
            "字符串内英文双引号须写成 \\\" 。\n"
            "根据用户**最新一句话**判断：是否需要本助手执行「多轮联网检索 + 结构化舆情长报告」"
            "（事件脉络、观点谱系、风险与信息缺口等章节）。\n\n"
            "**need_deep_research = true**：用户想分析某**品牌/人物/事件/政策/话题**的公开舆情、媒体报道、"
            "社交讨论、声量与情感等（即使用户表述简短，只要意图是舆情研判即可）。\n"
            "**need_deep_research = false**：纯闲聊、打招呼、你是谁/能做什么、感谢、与舆情研究无关的话题，"
            "或明显不需要检索与长报告的简单追问。\n\n"
            f"JSON Schema：\n{OpinionRouteDecision.model_json_schema()}\n"
            "字段 reason：用一两句中文说明为何如此判断。"
        )
        try:
            raw = await self._llm_plain(sys_txt, f"用户输入：\n{self.user_query}")
            return OpinionRouteDecision.model_validate(self._parse_json_object(raw))
        except (ValidationError, ValueError) as e:
            logger.warning("路由 JSON 解析失败，默认走深度研究: %s", e)
        except Exception as e:  # noqa: BLE001
            logger.warning("路由异常，默认走深度研究: %s", e, exc_info=True)
        return OpinionRouteDecision(
    ...
    async def reply(
        ...
        if structured_model is not None:
            logger.warning("OpinionDeepResearchAgent 忽略 structured_model，仅输出文本终稿。")
    ...
    def _retrieval_time_rules_for_llm(self) -> str:
        ...
        return (
            ...
            "- 用户若只说「近一周」「最新舆情」，检索词里**不必**硬加具体年份，以免搜到陈旧稿件。\n"
        )
    ...
    async def _phase_decompose(self) -> OpinionDecompose:
        sys_txt = (
            "你是舆情研究规划助手。根据用户问题，输出**仅包含一个 JSON 对象**的回复，"
            "不要 Markdown 代码围栏，不要在 JSON 前后加任何说明文字。\n"
            "重要：所有字符串内若出现英文双引号必须写成 \\\" ；各字段尽量简短，避免复杂标点。\n"
            f"{self._retrieval_time_rules_for_llm()}\n"
            "JSON 必须符合下列字段：\n"
            f"{OpinionDecompose.model_json_schema()}\n"
            "字段说明：knowledge_gaps=尚不清楚的点；working_plan=分步计划；"
            "initial_queries=3～8 条中文搜索查询。"
        )
        try:
            raw = await self._llm_plain(sys_txt, f"用户问题：\n{self.user_query}")
            return OpinionDecompose.model_validate(self._parse_json_object(raw))
        except (ValidationError, ValueError) as e:
            logger.warning("子任务分解解析或校验失败，使用兜底: %s", e)
        except Exception as e:  # noqa: BLE001
            logger.warning("子任务分解异常，使用兜底: %s", e, exc_info=True)
        return OpinionDecompose(
            knowledge_gaps="需通过联网检索核实事实与观点来源。",
            working_plan="1) 广度检索 2) 交叉核对 3) 归纳情感与话题 4) 成稿",
    ...
    async def _phase_follow_up(self) -> OpinionFollowUp:
        sys_txt = (
            "你是检索策略助手。根据已获得的检索摘录，判断是否需要继续搜索。"
            "只输出**一个 JSON 对象**，不要其它文字；字符串内英文双引号须写成 \\\" 。\n"
            f"{self._retrieval_time_rules_for_llm()}\n"
            "Schema：\n"
            f"{OpinionFollowUp.model_json_schema()}\n"
        ...
        try:
            ...
            return OpinionFollowUp.model_validate(self._parse_json_object(raw))
        except (ValidationError, ValueError):
            return OpinionFollowUp(
    ...
    async def _phase_intermediate_summary(self) -> str:
        sys_txt = (
            "你是研究助理。请将下列「程序化检索得到」的材料整理成**中间摘要**："
            "用中文分条列出可核实要点、潜在观点对立、信息缺口；不要编造未出现在材料中的事实。"
            "不要输出最终舆情报告，只输出摘录级摘要。"
        )
    ...
    def _build_final_user_content(self, intermediate_summary: str) -> str:
        return (
            "【编排阶段说明】以下内容由系统自动完成：子任务分解、多轮百炼联网搜索、"
            "是否续搜的决策与中间摘要。请你作为舆情分析助手，**严格依据**下列材料撰写"
            "完整终稿（Markdown），并遵守系统提示中的合规与章节要求；不得编造材料中"
            "不存在来源的细节。\n\n"
    ...
    def _pick_search_tool_spec(self) -> dict[str, Any]:
        forced = (os.getenv("OPINION_SEARCH_TOOL_NAME") or "").strip()
        ...
        if forced:
            for spec in schemas:
                if spec.get("function", {}).get("name", "") == forced:
                    return spec
            raise RuntimeError(
                f"未找到名为 {forced} 的工具，请检查 OPINION_SEARCH_TOOL_NAME。",
            )
