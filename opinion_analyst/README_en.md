# Opinion Analyst — Deep Research Assistant

A Deep Research opinion-analysis sample built on [Google ADK](https://github.com/google/adk-python): Bailian WebSearch MCP + multi-round web search and structured assessment. Output is for reference only and does not constitute legal, investment, or PR advice.
...
│   └── agent.py              # Agent definition (name="opinion_analyst")
├── configs/
...
1. **Deploy Now**: open the [Compute Nest Opinion Analyst deployment page](https://computenest.console.aliyun.com/agent/deploy/cn-hangzhou/OpinionAnalyst?serviceId=service-0682c63593ea443e900c&deployType=ECS&TemplateName=ECS%E7%89%88) and click "Deploy Now".
...
- **WebUI Access URL**: open `http://<instance-ip>:8000` and select the `opinion_analyst` app.
- **Code Debug Address** (cluster deployment): `http://<instance-ip>:8080` for online source viewing and debugging.
- **API Call Example**: send an SSE request to `http://<instance-ip>:8000/run_sse` with `app_name=opinion_analyst` and `user_id=user`. OpenAPI docs: `http://<instance-ip>:8000/docs`.
...
Open http://127.0.0.1:8000 , select the `opinion_analyst` app, and ask for an opinion research task; or verify the service is ready:

```bash
curl -fsS http://127.0.0.1:8000/list-apps
# Expected: ["opinion_analyst"]
```

**Example Prompts**

- Search recent two-week Chinese coverage and discussions about a brand, then output a structured opinion report (background, viewpoints, risks, information gaps)
