...
start() {
    if [ -f "$PID_FILE" ] && kill -0 "$(cat "$PID_FILE")" 2>/dev/null; then
        echo "ADK Browser Use is already running (PID $(cat "$PID_FILE"))"
        exit 1
    fi
    local adk_cmd
    adk_cmd="$(find_adk)"
    echo "Starting ADK Browser Use on ${HOST}:${PORT} ..."
...
stop() {
    if [ ! -f "$PID_FILE" ]; then
        echo "ADK Browser Use is not running (no PID file)"
        exit 1
    fi
    local pid
    pid=$(cat "$PID_FILE")
    if kill -0 "$pid" 2>/dev/null; then
        echo "Stopping ADK Browser Use (PID ${pid}) ..."
        kill "$pid"
        rm -f "$PID_FILE"
        echo "Stopped."
    else
...
status() {
    if [ -f "$PID_FILE" ] && kill -0 "$(cat "$PID_FILE")" 2>/dev/null; then
        echo "ADK Browser Use is running (PID $(cat "$PID_FILE"))"
    else
        echo "ADK Browser Use is not running"
    fi
    echo "Session: $(session_mode_label)"
}
