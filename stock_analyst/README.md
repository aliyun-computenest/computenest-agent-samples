...
基于 [AgentScope Agent Service](https://docs.agentscope.io/v2/deploy/agent-service.md) 的股票分析示例：百炼 WebSearch MCP + Deep Research 系统提示词。输出仅供参考，**非持牌证券投资咨询**。
...
1. **立即部署**：打开 [计算巢 股票智能分析助手 部署页](https://computenest.console.aliyun.com/agent/deploy/cn-hangzhou/StockAnalyst?serviceId=service-693904e6ce8943f49a3b&TemplateName=%E5%B8%82%E5%9C%BA%E6%A8%A1%E6%9D%BF)，点击「立即部署」。
...
- **API 调用示例**：向 `http://<实例IP>:8090/chat/` 发消息，请求头 `X-User-ID: demo_user`，body 需 `agent_id=stock_analyst` 与 `session_id`（可在 Web UI 查看，或 `POST /sessions/` 创建）。OpenAPI：`http://<实例IP>:8090/docs`。
...
另开终端运行 `client.py`；使用预置 `agent_id=stock_analyst`，每次运行会新建 Session。
